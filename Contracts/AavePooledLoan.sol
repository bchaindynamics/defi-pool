...
contract PoolLoanAave is Aave {
    ...
    function participate() external {
        require(
            ...
            "Sorry, you can't participate. Time out!"
        );
        require(
            totalParticipants + 1 <= maxParticipants,
            "Sorry max particiapnt is filled"
        );
        require(
            !isParticipant[msg.sender],
            "You are already a part of the pool"
        );
        require(
            token.transferFrom(
                msg.sender,
    ...
    function bid(uint256 bidAmount) public {
        ...
        require(
            ...
            "Auction is over, better luck next time"
        );
        require(
            isParticipant[msg.sender],
            "Please be a part of the pool to particiapte"
        );
        require(!takenLoan[msg.sender], "Sorry, you already have a loan");
    ...
    function claimLoan() public {
        ...
        require(isWinner, "Sorry you ain't the highest bidder");

        if (term < totalParticipants - 1) {
            require(
                term < getAuctionCount(),
                "Sorry, wait until auction is done to claim loan"
            );
        ...
        require(
            ...
            "Oops! Unable to withdraw! FAILED!"
        );
    ...
    function getAuctionCount() public view returns (uint256) {
        uint256 term = ((block.timestamp - poolStartTimestamp) /
            ((auctionInterval - auctionDuration) * 3 hours)) + 1;
    ...
    function nextAutionStartTimestamp() public view returns (uint256) {
        ...
        if (block.timestamp < poolCloseTimestamp() && totalParticipants > 1) {
            result =
                poolStartTimestamp +
                ((getAuctionCount() - 1) * auctionInterval * 3 hours);
        }
    ...
    function nextAutionCloseTimestamp() public view returns (uint256) {
        ...
        if (block.timestamp < poolCloseTimestamp() && totalParticipants > 1) {
            result = nextAutionStartTimestamp() + (auctionDuration * 3 hours);
        }
